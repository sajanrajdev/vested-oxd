...
def test_real_world_unlock(strat_proxy, sett_proxy, governance, want, whale, fish):
    ...
    with brownie.reverts("Withdrawal Safety Check"):
        sett_proxy.withdraw(
            modest_amount, {"from": whale}
        )  ## Can't withdraw as not unlocked
    ...
    assert (
        sett_proxy.getPricePerFullShare() == 1e18
    )  ## no increase in ppfs, just unlock
